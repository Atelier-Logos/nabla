...
mod control_flow_tests {
    use nabla_cli::enterprise::secure::control_flow::{
        ControlFlowGraph, BasicBlockType, EdgeType
    };
    ...
    fn test_call_graph_analysis() {
        ...
        assert!(call_graph.call_sites.len() >= 0);
        assert!(call_graph.function_summaries.len() >= 0);
    }
    ...
    fn test_basic_block_types() {
        ...
        match function_entry {
            BasicBlockType::FunctionEntry { function_name, address } => {
        ...
        match sequential {
            BasicBlockType::Sequential { start_address, end_address } => {
    ...
    fn test_build_with_large_binary() {
        ...
        analysis.binary_data = Some(vec![0x7f, 0x45, 0x4c, 0x46].into_iter()
    ...
    fn test_concurrent_analysis() {
        ...
        let handles: Vec<_> = (0..4).map(|_| {
            ...
            thread::spawn(move || {
                ControlFlowGraph::build_from_analysis(&analysis_clone)
            })
        }).collect();
    ...
    fn test_memory_usage() {
        ...
        assert!(cfgs.len() >= 0); // At least some should succeed
    }
}