...
async fn test_path_traversal_protection() {
    ...
    for malicious_path in malicious_paths {
        ...
        assert!(result.is_err(), "Path traversal should be blocked: {}", malicious_path);
        ...
        assert!(error_response.message.contains("Path traversal") || 
    ...
    for valid_path in valid_paths {
        ...
        if result.is_ok() {
            ...
            println!("❌ Valid path rejected: {} - {:?}", valid_path, result.unwrap_err());
        }
    }
}
...
async fn test_boundary_enforcement() {
    ...
    let result = validate_file_path("../../../etc/passwd");
    assert!(result.is_err(), "Path outside current directory should be rejected");
    
    // Test that valid paths within the directory are accepted
    let result = validate_file_path("test_file.txt");
    assert!(result.is_ok(), "Path within current directory should be accepted");
