...
fn create_test_ar_archive() -> Vec<u8> {
    ...
    file_header.extend_from_slice(b"120       "); // file size (10 bytes)
    file_header.extend_from_slice(b"`\n"); // end characters
    
    archive_data.extend_from_slice(&file_header);
    
    // File content (fake ELF object)
    let mut object_content = vec![0u8; 120];
    object_content[0..4].copy_from_slice(b"\x7fELF"); // ELF magic
    object_content[4] = 1; // 32-bit
    object_content[5] = 1; // Little endian
    // Add some fake strings
    object_content[80..100].copy_from_slice(b"GCC: (GNU) 9.4.0\0\0\0");
    ...
    file_header2.extend_from_slice(b"80        "); // file size
    file_header2.extend_from_slice(b"`\n"); // end characters
    
    archive_data.extend_from_slice(&file_header2);
    
    // Second file content
    let mut object_content2 = vec![0u8; 80];
    object_content2[0..4].copy_from_slice(b"\x7fELF");
    object_content2[50..70].copy_from_slice(b"main\0printf\0exit\0\0\0\0\0");
...
fn create_library_archive() -> Vec<u8> {
    ...
    for (filename, description) in object_files.iter() {
        ...
        file_header.extend_from_slice(b"200       ");
        file_header.extend_from_slice(b"`\n");
        ...
        content[0..4].copy_from_slice(b"\x7fELF");
        let desc_bytes = description.as_bytes();
        let copy_len = std::cmp::min(desc_bytes.len(), 50);
        content[100..100+copy_len].copy_from_slice(&desc_bytes[..copy_len]);
...
async fn test_enhanced_archive_parsing() {
    ...
    if !analysis.embedded_strings.is_empty() {
        ...
        let has_compiler_strings = embedded_string_combined.contains("GCC") ||
                                  embedded_string_combined.contains("GNU");
...
async fn test_archive_metadata_extraction() {
    ...
    let has_standard_symbols = analysis.detected_symbols.iter()
