...
async fn test_enhanced_cortex_m_firmware_parsing() {
    ...
    assert!(result.is_ok(), "ARM Cortex-M firmware analysis should succeed");
...
async fn test_cortex_m_vs_generic_arm() {
    ...
    let success_checks = vec![
        ("ARM Cortex-M firmware format detected", analysis.format == "arm-cortex-m-firmware"),
        ("ARM architecture detected", analysis.architecture.contains("arm") || analysis.architecture.contains("cortex")),
...
async fn test_cortex_m_rtos_detection() {
    ...
    assert_eq!(analysis.format, "arm-cortex-m-firmware");
    assert!(has_rtos_strings || has_driver_strings, "Should find RTOS or driver functionality");
}