...
async fn test_enhanced_wasm_parsing() {
    ...
    assert!(
        ...
        analysis.architecture == "webassembly",
...
async fn test_complex_wasm_parsing() {
    ...
    let success_checks = vec![
        ("WASM format detected", analysis.format == "application/wasm"),
        ("WASM architecture detected", analysis.architecture.contains("wasm")),
    ...
    assert_eq!(analysis.format, "application/wasm");
    assert!(analysis.architecture.contains("wasm") || analysis.architecture.contains("webassembly"));
}
...
async fn test_wasm_metadata_extraction() {
    ...
    let has_wasm_features = analysis.detected_symbols.iter()
