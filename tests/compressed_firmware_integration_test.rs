...
fn create_gzip_compressed_firmware() -> Vec<u8> {
    ...
    let fake_compressed_payload = [
        0x73, 0x2b, 0xca, 0x85, 0x68, 0x00, 0x82, 0xb4, 0x41, 0x36, 0x06, 0x2e, 0x16, 0xce, 0x92,
        0x4c, 0x36, 0x85, 0xb4, 0x40, 0x00, 0xb2, 0xd4, 0xa2, 0xd4, 0xe2, 0x12, 0x85, 0xf4, 0xdc,
        0xd4, 0xe2, 0x12, 0x2e, 0x06, 0x26, 0x16, 0x95, 0x68, 0x06, 0x26, 0x56, 0x16, 0x95, 0x68,
        0x81, 0x91, 0x95, 0x87, 0x68, 0x06, 0x26, 0xd6, 0x70, 0x71, 0x0c, 0x72, 0x71, 0x74, 0x71,
        0x09, 0x01, 0x00,
    ];
...
fn create_iot_compressed_firmware() -> Vec<u8> {
    ...
    while firmware_data.len() < 0x800 + 20 {
...
async fn test_enhanced_compressed_firmware_parsing() {
    ...
    assert!(
        result.is_ok(),
        "Compressed firmware analysis should succeed"
    );
...
async fn test_compressed_firmware_with_strings_parsing() {
    ...
    let success_checks = vec![
        (
            "Compressed firmware format detected",
            analysis.format == "compressed-firmware",
        ),
        (
...
async fn test_compressed_firmware_metadata_extraction() {
    ...
    if let Some(compressed_analysis) = analysis.metadata.get("compressed_firmware_analysis") {
        println!(
            "✅ Found compressed firmware-specific analysis: {}",
            compressed_analysis
        );
...
async fn test_compressed_firmware_vs_other_formats() {
    ...
    let regular_gzip =
        b"\x1f\x8b\x08\x00\x00\x00\x00\x00\x00\x03document.txt\x00just some regular text content";
