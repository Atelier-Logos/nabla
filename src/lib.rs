...
pub async fn run_server(port: u16) -> anyhow::Result<()> {
    ...
    let crypto_provider = crate::enterprise::crypto::CryptoProvider::new(config.fips_mode, config.fips_validation)?;
    ...
    let auth_layer = axum::middleware::from_fn_with_state(state.clone(), crate::middleware::validate_license_jwt);

    // Public routes (no auth)
    let public_routes = Router::new()
        .route("/health", axum::routing::get(crate::routes::binary::health_check));
    
    // Protected routes (with auth)
    let protected_routes = Router::new()
        .route("/binary/analyze", post(crate::routes::binary::upload_and_analyze_binary))
        .route("/binary/diff", post(crate::routes::binary::diff_binaries))
        .route("/binary/attest", post(crate::enterprise::attestation::attest_binary))
        .route("/binary/check-cves", post(crate::routes::binary::check_cve))
        .route("/binary/chat", post(crate::routes::binary::chat_with_binary))
